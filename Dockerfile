...
# ADD a directory called docker-projects inside the UBUNTU IMAGE where you will be moving all of these files under this 
# DIRECTORY to
ADD . /usr/local/docker-projects
...
RUN cd /usr/local/docker-projects && mvn assembly:assembly
...
CMD ["java", "-cp", "/usr/local/docker-projects/target/docker-projects-1.0.0-SNAPSHOT.jar", "com.docker.git.HelloWorld"]